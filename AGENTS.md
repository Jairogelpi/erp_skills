...
* **Siguiente paso:** auditar §  por § qué queda sin implementar de `AGENTS.md` — petición explícita del usuario ("mira que no falte ningún §").

### 2026-08-10 UTC — unidad 37: auditoría §-por-§ y cierre de todos los requisitos de software

* **Qué:** auditoría sistemática de `AGENTS.md` sección por sección, volcada en `docs/spec-coverage.md`, y cierre de los diez huecos que encontró. En dos tandas, por prioridad que el usuario fijó.
...
* **Los dos auditores quedan en el repositorio** (`scripts/audit_docs_coherence.py`, `scripts/audit_docs_claims.py`) con rutas relativas, para que esta comprobación sea repetible y no dependa de que yo me acuerde. `AGENTS.md` se excluye del veredicto a propósito: es bitácora append-only y sus entradas antiguas son historia, no contradicción.
