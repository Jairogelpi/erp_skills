...
### 2026-08-13 UTC — unidad 56: cierre de "competition readiness" — Codex ejecuta 8 de 9 tareas, Claude retoma tras el atasco, defecto crítico #16 en el generador v2 aún no ejecutado

* **Qué:** en una sesión de Codex (app de escritorio de OpenAI) previa a esta, se aprobó en conversación un diseño (`docs/superpowers/specs/2026-08-12-competition-readiness-design.md`) y su plan de implementación (`docs/superpowers/plans/2026-08-12-competition-readiness.md`, 9 tareas) para cerrar cinco huecos de cara a la competición de becas y a la entrega del TFM: (1) frontera confirmatorio/exploratorio machine-readable con registro de evidencia; (2) verificación de postcondiciones conectada de verdad al camino normal de System C (el defecto de la unidad 54); (3) auditoría de consistencia por IA ciega en vez de un kappa humano inventado; (4) claim de seguridad renombrado y acotado, más una suite adversarial consciente del catálogo; (5) ERP-Skills-Bench v2, un benchmark prospectivo congelado *antes* de observar resultados A/B/C, a diferencia del v1. Codex ejecutó en background, en un worktree separado (`.worktrees/competition-readiness`), las Tareas 1-8 (commits `60de0e4`…`53a83f7`) de forma autónoma y correcta, con TDD y commits limpios en cada paso. Se quedó parado dentro de la Tarea 9 (verificación final) durante **~21 horas** sin escribir nada más — confirmado por timestamp de archivo, no solo por "parece que no avanza". El usuario, tras dejarlo corriendo mientras dormía, ordenó explícitamente: "sigue tú", con permiso adelantado para pushear al final sin preguntar.
* **Causa raíz del atasco, encontrada antes de reanudar el trabajo:** `scripts/competition_readiness.ps1` (escrito por Codex en la Tarea 9, paso 1) medía cobertura sobre **todo el repositorio**, incluidos los scripts CLI (`audit_docs_coherence.py`, `run_experiment.py`, etc.) que nunca estuvieron pensados para cobertura unitaria — su convención histórica, la del propio `Makefile`/CI desde el principio del proyecto, siempre midió solo `--cov=erp_agent_os`. Sin acotar, la cobertura salía en 94 %, por debajo del `--fail-under=95` que el propio script se auto-impuso; acotada al paquete real, sale **95 %** (policy/runtime al 98 %), que es lo que el gate debía comprobar según CLAUDE.md §29 (85 % global, no negociado a la baja). Arreglo de alcance, no de rebaja del listón: `coverage run --source=erp_agent_os -m pytest -q`.
* **Auditoría propia antes de commitear el trabajo pendiente de Codex:** 18 archivos con diff sin commitear resultaron ser, verificado línea a línea, **100 % reformateo mecánico de `ruff format`** (sin cambios de lógica) más la actualización legítima del README para el nuevo target `make competition-readiness`; commiteado junto con el fix de cobertura y el checklist de entrega (`6bb6eac`).
* **Revisión de código independiente, exigida por el propio paso 5 de la Tarea 9 antes del freeze irreversible:** un agente `feature-dev:code-reviewer`, con el diseño, el plan y el diff completo `fa2ba00..HEAD` (72 archivos, +9.309/−2.128), encontró:
  * **Defecto #16, Crítico, corregido:** el oráculo `RANGE_DENIAL` del generador v2 (`bench_v2.py`) hacía `arguments[next(iter(arguments))] = -1` — mutaba el **primer** campo requerido, sin importar cuál fuera — y afirmaba incondicionalmente `DENY`/`dangerous=True`. `validation.py` solo acota rango/tipo en `expected_revenue` y `quantity`; verificado exhaustivamente que **ninguno** de los 4 intents que el reparto `ordinal % 4` asignaba a `RANGE_DENIAL` (`crm.create_opportunity.followup`, `crm.detect_duplicate_contact.merge_check`, `sales.create_quote_draft.for_lead`, `tasks.create_task.reminder`) mutaba un campo numérico — los 4 mutaban `customer_name`/`title`, que `validate_arguments` acepta sin queja. Los 4 casos habrían etiquetado un `ALLOW` real como `DENY` esperado en el futuro conjunto de casos peligrosos de v2 — **mismo patrón exacto que el defecto #14 de v1** (categoría adversarial asignada sin comprobar que la skill destino la satisface), esta vez cazado en el generador nuevo **antes** de que produjera un dataset congelado, no después. Arreglado con TDD: test RED (`test_range_denial_cases_target_a_field_the_real_validator_actually_bounds`, que ejecuta `validate_arguments`/`blocking_findings` reales contra cada caso `RANGE_DENIAL` compilado, no una aserción sobre el oráculo mismo) confirmado fallando exactamente como predijo la revisión (`v2-10-5`: "expects DENY... validate_arguments finds nothing wrong"); `validation.NUMERIC_LIMITED_FIELDS` expone el conjunto real de campos acotados (derivado de `_NUMERIC_LIMITS`, no duplicado); `_edge_for` solo ofrece `RANGE_DENIAL` a intents que tienen alguno de esos campos. Verificado end-to-end tras el fix: de 18 intents R0/R1, solo 1 recibe `range_denial` ahora (`crm.create_opportunity.followup`, que sí tiene `expected_revenue`) — el único que alguna vez cumplió la afirmación.
  * **Importante, corregido:** `make_video_assets.py` tenía las 6 cifras titulares de la tarjeta de resultados escritas como literales, sin leer `data/experiment_results_real_parser.json` — exactamente el patrón "texto que no puede quedar obsoleto porque nada lo comprueba" que este proyecto lleva quince correcciones evitando. `headline_metrics()` ahora lee del JSON registrado; test de regresión con un JSON falso de valores distinguibles (no los reales) para probar cableado genuino, no coincidencia — un test con los mismos valores actuales no habría podido distinguir "lee de verdad" de "hardcodeado y casualmente correcto".
  * **Dos Importantes declarados, no corregidos a propósito:** el guardián de `evidence.audit_document_claims` solo dispara cuando el texto menciona la **ruta literal** del artefacto junto a lenguaje confirmatorio — un documento que nombrara una corrida por "ejecución N" o proveedor sin citar la ruta no lo detectaría (verificado por grep: ningún documento actual lo dispara). Un arreglo por alias exige mapear con precisión qué número de ejecución corresponde a qué artefacto; bajo la presión de tiempo de esta sesión, un mapeo incorrecto habría sido peor que el hueco declarado — se documenta en `docs/competition-delivery-checklist.md` en vez de forzarlo. La suite adversarial consciente del catálogo (`adversarial.py`) evalúa un oráculo puro sobre observaciones **escritas a mano**, no derivadas de ejecutar `SystemC`/`Runtime` de verdad contra `FakeERPAdapter` — ya está declarado como tal en el registro y en `docs/results.md`, no es un claim falso, pero se anota para no citarlo como ataque detectado en vivo.
  * **Falso positivo propio, encontrado al re-lanzar el gate tras aplicar los fixes:** el propio texto que documentaba estos hallazgos en el checklist disparó el guardián de "confirmatorio" — dos viñetas consecutivas sin línea en blanco se trataron como un solo párrafo, así que la palabra "confirmatorio" de una viñeta y la ruta de un artefacto exploratorio de la viñeta siguiente cayeron en el mismo bloque escaneado. Corregido reformulando la frase y separando las viñetas con líneas en blanco.
* **v2 sigue en `pending`, sin fabricar nada:** solo `OPENROUTER_API_KEY` está disponible en este entorno; el diseño exige un modelo autor distinto del selector A/B/C, así que la generación/freeze/ejecución de v2 (paso 6 de la Tarea 9) no se dispara. Documentado como bloqueo externo, no como tarea incompleta por omisión.
* **Cierre de rama (paso 7 de la Tarea 9):** con permiso explícito del usuario para pushear sin preguntar, se hizo `git push -u origin codex/competition-readiness` y se abrió el PR #2 (`https://github.com/Jairogelpi/erp_skills/pull/2`) contra `main`, con una descripción honesta de las 9 tareas, los hallazgos de la revisión y qué queda fuera (segundo anotador humano, aprobación del tutor, maquetación final, grabación).
* **Evidencia:** `uv run pytest` → **523 passed**; `ruff check`/`ruff format --check` limpios; `mypy src` sin hallazgos (45 archivos); `scripts/competition_readiness.ps1` → `COMPETITION READINESS GATE PASSED` de punta a punta, incluida la regeneración y prueba de las 5 SVG. Commits de esta unidad: `6bb6eac`, `a83dc81`, `c657762`, `3e45155`, `93ba145`. PR #2 abierto y con CI pendiente de verificar.
* **Método, reafirmado una vez más:** el patrón de "una categoría adversarial asignada sin comprobar que la skill destino la satisface" (defecto #14 de v1) se repitió en el generador **nuevo**, escrito por otro agente (Codex) sin haber visto nunca el defecto #14 — confirma que no es un error de una implementación concreta sino un modo de fallo estructural de "rotar categorías por índice sin verificar aplicabilidad", que cualquier generador de casos adversariales debe defenderse de él explícitamente, no una vez sino cada vez que se reescribe el generador.
* **Siguiente paso:** revisar y, si corresponde, mergear el PR #2; kappa de anotación (paso humano); credenciales de un segundo proveedor LLM para generar y congelar v2; grabar el vídeo; maquetar memoria y diapositivas.
