...
| §26 | Solo **2 de las 12 skills** están mapeadas a modelos reales de Odoo (`crm.create_opportunity`, `crm.update_expected_revenue`); las otras 10 necesitarían su propio mapeo. Las tres demos (adaptador, pipeline gobernado y casos adversariales) están **ejecutadas y verificadas en vivo** contra una rama de desarrollo, con control positivo y guardián que rechaza producción y staging. |
